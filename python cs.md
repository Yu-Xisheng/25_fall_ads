...
选**结束最早**的区间——给后面留下最大自由度 ——也就是按右端点排序
...
首先，我们需要知道stack通常对应下标，那么单调栈就是为了准备一个随侍侯选的有序队列，方便调度
...
1. 用单调栈找：
   - 左侧最近严格小于arr[i]
   - 右侧最近小或等于arr[i]（防止在同一个贡献区间内最小值出现两次导致重复计算，故直接将这个区间分给两个最小值）
2. 计算每个元素的贡献
...
这其实就是把找到每个元素作为最小值时分别对应的最长连续子数组
...
因此，将**DFS**（**Depth-First Search**，**深度优先搜索**）与**递归、回溯**放在一起来理解
...
在面对dfs+backtrack+recursion的情况中，显然会因为深度深以及状况过多的情况导致爆炸，这种时候就需要减去不必情况
...
二：可以用二进制的拆分方法，将每个物品的数量拆分成1+2+4+8+...+余项
